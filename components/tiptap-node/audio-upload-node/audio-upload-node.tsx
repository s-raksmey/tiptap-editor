...
export function AudioUploadNode(props: NodeViewProps) {
  const {
    node,
    updateAttributes,
    extension,
  ...
  const accept =
    (node.attrs.accept as string) || extension.options.accept || "audio/*";
  ...
  const label = useMemo(() => filename || "Audio file", [filename]);
  ...
  if (src) {
    return (
      <NodeViewWrapper
        className={`tiptap-audio-upload-node ${selected ? "ProseMirror-selectednode" : ""}`}
        data-audio-upload-node=""
      >
        <div className="tiptap-audio-upload-node__header">
          <div className="tiptap-audio-upload-node__meta">
            <span className="tiptap-audio-upload-node__title">{label}</span>
          </div>

          <div className="tiptap-audio-upload-node__actions">
            <button
              type="button"
              className="tiptap-audio-upload-node__action"
        ...
        <audio className="tiptap-audio-upload-node__media" controls src={src} />
  ...
  return (
    ...
    >
      <div
        className={`tiptap-audio-upload-node__dropzone ${isDragging ? "drag-over" : ""}`}
        onDragEnter={handleDragEnter}
        onDragLeave={handleDragLeave}
        onDragOver={handleDragOver}
        onDrop={handleDrop}
      >
        <div className="tiptap-audio-upload-node__content">
          <div className="tiptap-audio-upload-node__dropzone-file">
            <FileIcon />
            <FileCornerIcon />
            <div className="tiptap-audio-upload-node__icon-badge">
              <CloudUploadIcon />
            </div>
          </div>

          <div className="tiptap-audio-upload-node__headline">
            <span className="tiptap-audio-upload-node__headline-link">
              Click to upload
            </span>{" "}
            or drag and drop
          </div>

          <div className="tiptap-audio-upload-node__helper">
            {isUploading ? `Uploading ${progress}%` : helperText}
          </div>

          {errorMessage ? (
            <div className="tiptap-audio-upload-node__error">
              {errorMessage}
            </div>
          ) : null}
        </div>
